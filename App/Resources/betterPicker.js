function BetterPicker(args) {
    ...
    rightView.addEventListener("click", function() {
        if (rightView.dataPosition < self.data.length) {
            ...
            rightView.text = rightView.dataPosition == self.data.length ? "" : self.data[rightView.dataPosition].text;
        }
    });
    leftView.addEventListener("click", function() {
        if (leftView.dataPosition >= 0) {
            ...
            leftView.dataPosition = leftView.dataPosition - 1;
            rightView.dataPosition = rightView.dataPosition - 1;
            leftView.text = 0 > leftView.dataPosition ? "" : self.data[leftView.dataPosition].text;
        }
    });
